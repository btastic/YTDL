...
public class CueSheet
{
    ...
    /// </summary>
    public List<string> Garbage { get; private set; } = new List<string>();
    ...
    {
        get { return m_Tracks.ToImmutableList(); }
    }
    ...
    private void RemoveEmptyLines(ref string[] file)
    {
        ...
        {
            if (!string.IsNullOrEmpty(file[i].Trim()))
            {
        ...
        for (int i = 0; i < file.Length; i++)
        {
            ...
            switch (file[i].Substring(0, file[i].IndexOf(' ')).ToUpper())
            {
                ...
                case "TRACK":
                    ...
                    {
                        m_Tracks.ElementAt(trackOn).DataFile = currentFile;
    ...
    {
        m_Tracks.Add(new Track(tracknumber, datatype));
    }
    ...
    {
        var track = new Track(m_Tracks.Count() + 1, "")
        {
            Performer = performer,
            Title = title
        };
    ...
    {
        var track = new Track(m_Tracks.Count() + 1, datatype)
        {
            Performer = performer,
            Title = title
        };
    ...
    public string CalculateCDDBdiscID()
    {
        ...
        AnsiConsole.WriteLine(n.ToString());

        t = ((lastTrackIndex(m_Tracks[m_Tracks.Count - 1]).Minutes * 60) + lastTrackIndex(m_Tracks[m_Tracks.Count - 1]).Seconds) -
            ((lastTrackIndex(m_Tracks[0]).Minutes * 60) + lastTrackIndex(m_Tracks[0]).Seconds);

        ulong lDiscId = (((uint)n % 0xff) << 24 | (uint)t << 8 | (uint)m_Tracks.Count);
